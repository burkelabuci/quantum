...
def create_folder_and_generate_filename_csv(base_path=default_path):
...
def create_folder_and_generate_filename_lockin(base_path=default_path):
    # Get today's date in mmddyy format
    today_date = datetime.now().strftime("%m%d%y")
    ...
    # Iterate through existing files to find the next available file_number
    while True:
        filename = f"SR830_lockin{month}{day}{year}{file_number:04d}.txt"
        if filename not in existing_files:
            break
        file_number += 1
